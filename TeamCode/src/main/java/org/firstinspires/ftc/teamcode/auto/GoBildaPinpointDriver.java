...
public class GoBildaPinpointDriver extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
    ...
    private static final float goBILDA_SWINGARM_POD = 13.26291192f * (1/(1/25.4f)); //ticks-per-in for the goBILDA Swingarm Pod
    private static final float goBILDA_4_BAR_POD    = 19.89436789f * (1/(1/25.4f)); //ticks-per-in for the goBILDA 4-Bar Pod
    ...
    public void setEncoderResolution(GoBildaOdometryPods pods){
        if (pods == GoBildaOdometryPods.goBILDA_SWINGARM_POD) {
            writeByteArray(Register.IN_PER_TICK, (floatToByteArray(goBILDA_SWINGARM_POD, ByteOrder.LITTLE_ENDIAN)));
        }
        if (pods == GoBildaOdometryPods.goBILDA_4_BAR_POD){
            writeByteArray(Register.IN_PER_TICK,(floatToByteArray(goBILDA_4_BAR_POD, ByteOrder.LITTLE_ENDIAN)));
        }
    }
    ...
    public void setEncoderResolution(double ticks_per_mm){
        writeByteArray(Register.IN_PER_TICK,(floatToByteArray((float) ticks_per_mm,ByteOrder.LITTLE_ENDIAN)));
    }
